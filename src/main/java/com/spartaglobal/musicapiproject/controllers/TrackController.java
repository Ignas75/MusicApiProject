...
public class TrackController {
    ...
    public ResponseEntity createTrack(@RequestHeader("Authorization") String authTokenHeader, @RequestBody Track newTrack){
        ...
        if(authorization.isAuthorizedForAction(token,"chinook/track/create")){
    ...
    public ResponseEntity updateActor(@RequestBody Track newState, @RequestHeader("Authorization") String authTokenHeader){
        String token = authTokenHeader.split(" ")[1];
        if(authorization.isAuthorizedForAction(token,"chinook/track/create")){
            return new ResponseEntity<>("Not Authorized", HttpStatus.UNAUTHORIZED);
        }
